...
// Выполняем запрос к базе данных для получения информации о фильме
$sql_movie = "SELECT * FROM movies WHERE title = 'Во все тяжкие'";
$conn = include('../database_connect/database.php');
...
// Выполняем запрос к базе данных для получения отзывов
$sql_reviews = "SELECT * FROM reviews WHERE movie_title = 'Во все тяжкие'";
...
if ($result_reviews->num_rows > 0) {
    ...
    while ($row_review = $result_reviews->fetch_assoc()) {
        ...
        echo '<div class="panel-heading"><i class="glyphicon glyphicon-user"><span>Сергей</span></i></div>';
